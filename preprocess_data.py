...
class RawDataset:
    def __init__(
        self, root, unit="VG4", load_training=False, load_synthetic=False
    ) -> None:
        read_pq_file = lambda f: pq.read_table(root / f).to_pandas()
        cases = {
            "test": [
                f"{unit}_generator_data_testing_real_measurements.parquet",
                root / f"{unit}_generator_data_testing_real_info.csv",
            ],
        }
        if load_training:
            cases["training"] = [
                f"{unit}_generator_data_training_measurements.parquet",
                root / f"{unit}_generator_data_training_info.csv",
            ]
        if load_synthetic:
            cases["test_synthetic"] = [
                f"{unit}_generator_data_testing_synthetic_01_measurements.parquet",
...
def extracting_name_groups_time_series(rds, set_type):
    ...
    return (
        control_time_series_name["attribute_name"],
        sensors_time_series_name["attribute_name"],
    )


def extracting_groups_time_series(rds, set_type):
    # Extract names of control and sensor time series
    control_time_series_name, sensors_time_series_name = (
        extracting_name_groups_time_series(rds, set_type=set_type)
    )
    ...
    missing_columns = [
        col for col in Operating_modes_name if col not in available_columns
    ]
    if missing_columns:
        print(
            f"        Warning: Missing operating mode columns for {set_type}: {missing_columns}"
        )
...
def get_indexes_operating_mode(operating_modes_df, operating_conditions):
    ...
    for column, value in operating_conditions.items():
        if column in operating_modes_df.columns:
            ...
            print(
                f"        Warning: Column '{column}' is missing from operating_modes_df and will be skipped."
            )
...
def dynamic_group_features(df):
    feature_groups = {
        "voltage_current": df.columns[
            df.columns.str.startswith(("exc", "ph", "elec", "mid", "neutral"))
        ].tolist(),
        "stator_temperature": df.columns[
            df.columns.str.startswith("stat_") & df.columns.str.contains("coil|magn")
        ].tolist(),
...
def preprocess_hydraulic_data(rds, set_type, operating_conditions):
    control_df, sensors_df, operating_modes_df = extracting_groups_time_series(
        rds, set_type=set_type
    )
    ...
    water_opening_features = [
        col for col in control_df.columns if "water" in col and "opening" in col
    ]
    control_df = create_scaled_sum_feature(
        control_df, water_opening_features, "water_opening_sum"
    )
    injector_opening_features = [
        col for col in control_df.columns if "injector" in col and "opening" in col
    ]
    control_df = create_scaled_sum_feature(
        control_df, injector_opening_features, "injector_opening_sum"
    )
    ...
    selected_sensor_data = [
        sensors_df[grouped_data[key]] for key in grouped_data.keys()
    ]
...
def save_preprocessed_data(
    base_dir, unit, mode, set_type, operation, control_df, sensors_df
):
...
def main():
    ...
    for unit in units:
        ...
        if unit == "VG4":
            ...
            rds = RawDataset(
                dataset_root, unit, load_training=True, load_synthetic=False
            )
        else:
            print(f"  Loading data for {unit} (training and synthetic)...")
            rds = RawDataset(
                dataset_root, unit, load_training=True, load_synthetic=True
            )
            ...
            for set_type in rds.data_dict.keys():
                ...
                try:
                    ...
                    rds_processed = preprocess_hydraulic_data(
                        rds,
                        set_type=set_type,
                        operating_conditions=current_operating_conditions,
                    )
                    ...
                    for period_idx, period_data in enumerate(
                        operating_periods, start=1
                    ):
